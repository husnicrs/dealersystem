	...
	function show($rptdatefrom, $rptdateto){
        ...
        $sql = "SELECT
				...
				WHERE salesvehicle.status=0
				AND salesvehicle.datetrans BETWEEN :rptdatefrom AND :rptdateto AND salesvehicle.clearstatus=0  AND salesvehicle.paytypeid=2";
...
function showtotal($rptdatefrom, $rptdateto){
        ...
        $sql = "SELECT
				...
				FROM
					salesvehicle
				WHERE
					salesvehicle.status = 0
				AND salesvehicle.datetrans BETWEEN :rptdatefrom
				AND :rptdateto
				AND salesvehicle.clearstatus = 0  AND salesvehicle.paytypeid=2";
        $arrData = array(':rptdatefrom' => $rptdatefrom, ':rptdateto' => $rptdateto);
        $this->data = $this->db->searchData($sql, $arrData);
        return $this->data;
    }
