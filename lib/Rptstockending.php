	...
	function show(){
        $sql = "SELECT inventory.idinventory AS idkey, inventory.inventorycode, inventory.inventoryname, inventory.stockunit,  rackpurchase.rackno, SUM(DERIVEDTBL.qty) AS qty 
				FROM 
				(SELECT
				...
				GROUP BY inventory.idinventory, inventory.inventorycode, inventory.inventoryname, inventory.stockunit";
...
function showtotal(){
        ...
        $sql = "SELECT SUM(DERIVEDTBL.qty) AS tqty 
				FROM 
				(SELECT
				salesorderdet.inventoryid,
				-salesorderdet.quantity AS qty
