...
var modulename = "salesreturnpart";
var moduledesc = "Retur Penjualan Sparepart";
...
$(function(){
	...
	$('#dg').datagrid({
		onBeginEdit:function(rowIndex){
			...
			q1.add(q2).add(q3).add(n2).add(n3).add(n4).add(n6).numberbox({
				onChange:function(){
				var cost2 = (q3.numberbox('getValue')*n2.numberbox('getValue')-q3.numberbox('getValue')*n2.numberbox('getValue')*n3.numberbox('getValue')/100)-n6.numberbox('getValue');
...
function getTotalamount(){
	var data = $('#dg').datagrid('getData');
 	 ...
 	 //$('#brutoamount').textbox('setText', sum);
	 $("#brutoamount").val(sum);
}
...
function calculatenett() {
	var brutoamountsw = document.getElementById('brutoamount').value;
	var result = document.getElementById('brutoamountsw');
...
window.onload= function(){
   ...
   n3.onblur= function(){
	...
	n4= document.getElementById('discamtsw');
    //n4.value='';
    n4.onkeyup=n4.onchange= function(e){
        e=e|| window.event; 
        var who=e.target || e.srcElement,temp4;
        if(who.id==='discamt')  temp1= validDigits(who.value,0); 
        else temp4= validDigits(who.value);
        who.value= addCommas(temp4);
    }   
   n4.onblur= function(){
        var 
...
function calculatediscamt() {
	var discamtsw = document.getElementById('discamtsw').value;
	var result1 = document.getElementById('discamt');
		var rsdiscamt = (discamtsw);
		result1.value = rsdiscamt.replace(/,/g, "");
}

var myChangeDisc = setInterval(function(){ calculatechangedisc() }, 2000);

function calculatechangedisc() {
	var brutoamount = document.getElementById('brutoamount').value;	
	var disc = document.getElementById('disc').value;
	var discamt = document.getElementById('discamt').value;
	var result = document.getElementById('totalamount');
		var hasiltotal = brutoamount - discamt - (brutoamount * disc/100)
		if (!isNaN(brutoamount) && !isNaN(disc)) {
		result.value = Math.ceil(hasiltotal/100)*100;
		}
}
