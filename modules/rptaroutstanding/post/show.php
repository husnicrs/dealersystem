<?php
	...
	$lib='../../../lib/';
	$aColumns = array( 'idkey','datetrans','arbillingdate','cusname','vehicletypesw','mechineno','otrsw','penagihansw','leasing','pono','creditnettdpsw','salesmanname','leadsales','leadar'); 
	$sIndexColumn = 'idkey';
	$sTable = '(SELECT
				...
				users
				WHERE users.username="'.$user.'" AND salesvehicle.datetrans BETWEEN users.begindate AND users.enddate AND salesvehicle.status=1 AND salesvehicle.clearstatus=0) DERIVEDTBL'; 
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		...
		$row = array( $aRow['datetrans'], $aRow['arbillingdate'], $aRow['cusname'], $aRow['vehicletypesw'], $aRow['mechineno'], $aRow['otrsw'], $aRow['penagihansw'], $aRow['leasing'], $aRow['pono'], $aRow['creditnettdpsw'], $aRow['salesmanname'], $aRow['leadsales'], $aRow['leadar']);
		$output['aaData'][] = $row;
	}
