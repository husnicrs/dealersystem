...
var moduledesc = "Service Kendaraan";
...
{
	$('#dgservice').edatagrid({
		url:'modules/' + modulename + '/post/service/get.php?key=' + key,
		saveUrl: 'modules/' + modulename + '/post/service/save.php?key=' + key,
		updateUrl: 'modules/' + modulename + '/post/service/update.php',
		destroyUrl: 'modules/' + modulename + '/post/service/destroy.php',
		
		onBeforeSave: function(index){
			
			var vehicleservice = $(this).edatagrid('getEditor', {
				index: index,
				field: 'vehicleserviceid'
			});
						
			var row = $(this).edatagrid('getRows')[index];
			row.vehicleservicename = $(vehicleservice.target).combobox('getText');
...
$(function(){
	...
	$('#dg').datagrid({
		onBeginEdit:function(rowIndex){
			...
			var n5 = $(editors[7].target);
			var n4 = $(editors[8].target);
			
			q1.add(q2).add(q3).add(n2).add(n3).add(n4).add(n5).numberbox({
				onChange:function(){
				var cost2 = (q3.numberbox('getValue')*n2.numberbox('getValue')-q3.numberbox('getValue')*n2.numberbox('getValue')*n3.numberbox('getValue')/100)-(n5.numberbox('getValue'));
...
$(function(){
	...
	$('#dgservice').datagrid({
		onBeginEdit:function(rowIndex){
			var editors = $('#dgservice').datagrid('getEditors', rowIndex);
			var q1 = $(editors[1].target);
			var q2 = $(editors[2].target);
			var n1 = $(editors[3].target);
			var q3 = $(editors[4].target);
						
			q1.add(q2).add(q3).add(n2).add(n1).numberbox({
				onChange:function(){
				var cost2 = (q1.numberbox('getValue'))-(q1.numberbox('getValue')*q2.numberbox('getValue')/100)-(n1.numberbox('getValue'));
					q3.numberbox('setValue',cost2);
...
function getTotalamount(){
	var data = $('#dgservice').datagrid('getData');
	  var rows = data.rows;
	  var sum = 0;
	  for (i=0; i < rows.length; i++) {
		//sum+=rows[i].amount;
		var str = rows[i].subtotal;
	 ...
	 $("#serviceamount").val(sum);
}
var myVarnett = setInterval(function(){ calculatenett() }, 2000);
function calculatenett() {
	var totalamountsw = document.getElementById('serviceamount').value;
	var result = document.getElementById('serviceamountsw');
		var rstotalamount = (totalamountsw);
		result.value = rstotalamount.replace(/,/g, "");
}

//Calculate sparepart
var myVar = setInterval(function(){ getTotalamount1() }, 1000);
function getTotalamount1(){
	var data = $('#dg').datagrid('getData');
	  ...
	  // just to show if the sum is OK
 	 //$('#totalamount').textbox('setText', sum);
	 $("#sparepartamount").val(sum);
}
var myVarnett = setInterval(function(){ calculatenett1() }, 2000);
function calculatenett1() {
	var totalamountsw = document.getElementById('sparepartamount').value;
	var result = document.getElementById('sparepartamountsw');
		var rstotalamount = (totalamountsw);
		result.value = rstotalamount.replace(/,/g, "");
}

//Calculate service+sparepart
var myVarnett = setInterval(function(){ calculateservicepart() }, 2000);
function calculateservicepart() {
	var serviceamount = document.getElementById('serviceamount').value;	
	var sparepartamount = document.getElementById('sparepartamount').value;
	var result = document.getElementById('totalamount');
		var rstotal = eval(serviceamount) + eval(sparepartamount)
		if (!isNaN(serviceamount) && !isNaN(sparepartamount)) {
		result.value = rstotal;
		}
}
var myVarnett = setInterval(function(){ calculatenett2() }, 2000);
function calculatenett2() {
...
function refreshgridservice()
{
	$('#dgservice').datagrid('reload'); 
	getTotalamount();
}
