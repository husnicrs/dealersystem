<?php
	...
	$sTable = '(SELECT
				...
				users
				WHERE users.username="'.$user.'" AND salesvehicle.datetrans BETWEEN users.begindate AND users.enddate AND salesvehicle.status=0 AND salesvehicle.clearstatus=0 AND salesvehicle.paytypeid=2) DERIVEDTBL'; 
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		...
		$row = array( $aRow['datetrans'], $aRow['cusname'], $aRow['vehicletypesw'], $aRow['mechineno'], $aRow['otrsw'], $aRow['penagihansw'], $aRow['leasing'], $aRow['pono'], $aRow['creditnettdpsw'], $aRow['salesmanname'], $aRow['leadsales']);
		$output['aaData'][] = $row;
	}
