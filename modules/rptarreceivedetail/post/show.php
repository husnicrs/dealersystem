<?php
	...
	$lib='../../../lib/';
	$aColumns = array( 'idkey','datetrans','dateclear','cusname','address','vehicletypesw','mechineno','salesmanname','penagihansw','pencairansw','cairschemasw','selisihsw','leadsales','leasing'); 
	$sIndexColumn = 'idkey';
	$sTable = '(SELECT
				...
				users
				WHERE users.username="'.$user.'" AND salesvehicle.datetrans BETWEEN users.begindate AND users.enddate AND salesvehicle.clearstatus=1) DERIVEDTBL'; 
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		...
		$row = array( $aRow['datetrans'], $aRow['dateclear'], $aRow['cusname'], $aRow['address'], $aRow['vehicletypesw'], $aRow['mechineno'], $aRow['salesmanname'], $aRow['penagihansw'], $aRow['pencairansw'], $aRow['cairschemasw'], $aRow['selisihsw'], $aRow['leadsales'], $aRow['leasing']);
		$output['aaData'][] = $row;
	}
