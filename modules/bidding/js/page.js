...
var modulename = "bidding";
var moduledesc = "Bidding";
...
function LinkField(value,row) {
	var url = 'winopen.php?mod=bidding_subdetail&key='+row.idtransdet;
	//window.open(myurl, "myWindow", " height = 300, width = 300, resizable = 0")
	return '<a onclick="PopupCenterDual(\'' + url + '\',\'NIGRAPHIC\',\'1200\',\'500\'); " href="javascript:void(0);">Bidding</a>'
}
...
function showsubdetail()
{
	$('#dg').edatagrid({
		...
		saveUrl: 'modules/' + modulename + '/post/subdetail/save.php?key=' + key,
		updateUrl: 'modules/' + modulename + '/post/subdetail/update.php',
		destroyUrl: 'modules/' + modulename + '/post/subdetail/destroy.php',
		
		onBeforeSave: function(index){
			
			var supplier = $(this).edatagrid('getEditor', {
				index: index,
				field: 'supplierid'
			});
			var row = $(this).edatagrid('getRows')[index];
			row.suppliername = $(supplier.target).combobox('getText');
			
			var currency = $(this).edatagrid('getEditor', {
				index: index,
				field: 'currency'
			});
			var row = $(this).edatagrid('getRows')[index];
			row.currencyname = $(currency.target).combobox('getText');
