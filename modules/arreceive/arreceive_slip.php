<?php
	$db=new Database();
	$cashreceive = new Arreceive();
	$core=new Core();
	if (isset($_GET['key'])) {
		$key = $_GET['key'];
		$data = $cashreceive->showListHeader($key);
		extract($data);
		$datadetail = $cashreceive->showslipdetail($key);
		$datatotal = $cashreceive->showsliptotal($key);
	}
?>
<style type="text/css">
...
<table border=0 cellpadding=0 cellspacing=0 width=816 style='border-collapse:
 ...
 <tr height=22 style='mso-height-source:userset;height:16.5pt'>
  ...
  <td colspan=2 class=xl7212617 width=309 style='width:232pt'>: <?php echo $customername; ?></td>
  <td class=xl7012617 width=154 style='width:116pt'></td>
  <td colspan=2 class=xl7212617 width=182 style='width:137pt'></td>
  <td class=xl7012617 width=64 style='width:48pt'></td>
 </tr>
