<?php
	...
	$lib='../../../lib/';
	$aColumns = array( 'idkey','dodate','dono','vehlicemodel','vehicletype','mechineno','rangkano','vehiclecolor','year','cc','otr','channelname','unitprice'); 
	$sIndexColumn = 'idkey';
	$sTable = '(SELECT
				...
				WHERE users.username="'.$user.'" AND vehiclestock.dodate BETWEEN users.begindate AND users.enddate) DERIVEDTBL'; 
	...
	while ( $aRow = $rResult->fetch_assoc() ) {
		...
		$row = array( $aRow['dodate'], $aRow['dono'], $aRow['vehicletype'], $aRow['mechineno'], $aRow['rangkano'], $aRow['vehiclecolor'], $aRow['year'], $aRow['unitprice'], $aRow['channelname']);
		$output['aaData'][] = $row;
	}
